...
def work():
    ...
    os.system("mkdir -p ../android/PythonForAndroidInOne/libs/mips")
    XML=open("../android/PythonForAndroidInOne/res/raw/files.xml", "w")
    MIPSABI="../android/PythonForAndroidInOne/libs/mips"
    
    os.system("rm %s/*.so" % MIPSABI)
    ...
    for f in z.filelist:
        ...
        print z.extract(f, MIPSABI)
        i=i+1

    XML.write('<file src="lib%04i.so" target="lib/python2.6/python.zip"/>\n' % i)
    XML.write('</files>\n')
    os.system("cp python_extras_%s.zip %s/lib%04i.so" % ( EXTRA_VERSION, MIPSABI, i ) )
    XML.close()
